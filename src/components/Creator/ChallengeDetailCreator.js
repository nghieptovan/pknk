...
export default class ChallengeDetailCreator extends Component {
  ...
  componentWillMount(){
    ...
    const nameChallengeReply = challengeName.replace(/dotC/g,'.').replace(/%20/g,' ');
  ...
  render() {    
    ...
    return (
      <div id="container">        
          ...
          <ChallengeDetailCreatorBody error={this.state.error} loadingUpload={this.state.loadingUpload} dashboard={this.state.dashboard}/> 
