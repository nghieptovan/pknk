...
@connect(
  ...
  ({
    ...
    getListUploads: (challengeId, token, status, page, maxRecords) => ContentActions.getListUploads(challengeId, token, status, page, maxRecords),
...
export default class Uploads extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    let oldChallengeList = challenge.challenges, oldContentList = content.listUpload, newChallengeList = challenge.challenges, newContentList = content.listUpload;
    if(content.ratingMultiStatus !== nextContent.ratingMultiStatus && nextContent.ratingMultiResult){
      let { listUpload } = this.state;
      listUpload.map(data => {        
        nextContent.ratingMultiResult.map(data2 =>{
            if (data.id == data2.id){
              data.star = data2.star;  
              data.statusDate = data2.statusDate;
      ...
      this.setState({ confirmRating: true, listUpload, listUploadOriginal:  listUpload, listAll:  listUpload, listRating:  listUpload, listFlag:  listUpload, listSource: listUpload, listReward:  listUpload,  flagValue: [], ratingValue: [], rewardValue: [], sourceValue: [], loading: false  });
      setTimeout(() => {
        this.setState({ ratingMultiAll: false, confirmRating: false, loading: false, selectedUpload: [] });
      }, 1000);
    }  
    if(content.updatedRewardStatus !== nextContent.updatedRewardStatus && nextContent.updatedRewardStatus === 1){
      let { listUpload } = this.state;
      listUpload.map(data => {        
          nextContent.listUpload.map(data2 =>{
            if (data.id == data2.id){
              data.status = data2.status;
              data.statusDate = data2.statusDate;
              data.rewardLevel = data2.rewardLevel;
              data.rewardType = data2.rewardType;
              data.couponCode = data2.couponCode;              
            }
          })          
      })   
      this.setState({ listUpload, listUploadOriginal:  listUpload, listAll:  listUpload, listRating:  listUpload, listFlag:  listUpload, listSource:  listUpload, listReward:  listUpload, flagValue: [], ratingValue: [], rewardValue: [], sourceValue: [] });
    }  
    if (content.updatedReview !== nextContent.updatedReview && nextContent.updatedReviewContent) {
      let { listUpload } = this.state;
      listUpload.map(data => {        
          nextContent.updatedReviewContent.map(data2 =>{
            if (data.id == data2.id){
              data.star = data2.star;
              data.flagged = data2.flagged;
              data.comment = data2.comment;
              data.favourites = data2.favourites;
              data.status = data2.status;
              data.statusDate = data2.statusDate;
            }
          })          
    ...
    if (challenge.getListLoadForUpload != nextChallenge.getListLoadForUpload && nextChallenge.challenges.length > 0) {      
      let item = sessionStorage.getItem('contentStatus');
      item ? this.setState({activeType: item},()=>this.props.getListUploads(0, token, item, 0, this.state.maxRecords)) : '';        
      ...
      this.setState({ challengeId: challengeId, currentChallenge: currentChallenge, listChallenges: challengeList, ratingValue: [], flagValue: [], rewardValue: []},
        () => this.props.getListUploads(challengeId, token, this.state.activeType, 0 , this.state.maxRecords)); 
    }
    if (challenge.challenges !== nextChallenge.challenges) {
      newChallengeList = nextChallenge.challenges;
    }
    let { pageIndex } = this.state;
    if (content.getList !== nextContent.getList &&  nextContent.listUpload) {       
      if(this.state.challengeChanged || this.state.uploadChanged) {      
        newContentList = nextContent.listUpload;
        this.setState({challengeChanged: false, uploadChanged: false});
      } else {
        newContentList = _.concat(this.state.listUpload, nextContent.listUpload);
      }
      nextContent.listUpload.length > 0 ? pageIndex++ : '';     
    }
    if(!this.checkExpire()){
    ...
    challenge.loadingIcon !== nextChallenge.loadingIcon && nextChallenge.loadingIcon == 2 && nextChallenge.challenges.length == 0 && content.firstLoading ? this.setState({loadingUpload: false, loading: false}) :'';
    
    content.loadingContentIcon !== nextContent.loadingContentIcon && nextContent.loadingContentIcon != 1 ? this.setState({loadingUpload: false, loading: false}) :'';
    }else{
      this.setState({loadingUpload: false});
        }
    if(newChallengeList != oldChallengeList )      {
      this.setState({ listChallenges: newChallengeList});
    }   
    if(newContentList != oldContentList) {     
      this.setState({ listUpload: newContentList, pageIndex, isCallAPI: nextContent.listUpload.length > 0 ? false : true, error: '', listUploadOriginal: newContentList, listAll: newContentList, listRating: newContentList, listFlag: newContentList, listSource: newContentList, listReward: newContentList, ratingValue: [], flagValue: [], rewardValue: []}, () => {this.state.inputSearch ? this.handleSearch(): ''});      
    }    
  }
checkExpire = () => {
  ...
  handleSelection = (keys) => {   
    let { selectedUpload, listUpload } = this.state;
    _.forEach(keys , (key) => {  
      if(key.challengeStatus != 'ARCHIVED'){ 
    ...
    _.forEach(selectedUpload , (upload) => { 
      const selected = _.findIndex(listUpload, {'id' : upload.id});
      selected > -1 ? this.state.listUpload[selected].selectedData = true : '';
    });
  ...
  changeUploadsType = (value) => {
    ...
    this.setState({ activeType: value, selectedUpload: [] , loadingUpload: true, uploadChanged: true, pageIndex: 0 }, () => this.props.getListUploads(challengeId, token, value, 0, this.state.maxRecords));
  }
  handleSelect = (challenge) => {    
    const { token } = getCurrentUser();    
    this.setState({ challengeId: challenge.id, currentChallenge: challenge, selectedUpload: [], loadingUpload: true, challengeChanged: true, pageIndex: 0});
    this.props.getListUploads(challenge.id, token, this.state.activeType, 0 , this.state.maxRecords);   
  }
  ...
  selectCheckbox = (data) => {
    ...
    if(selectedUpload.length > 0){
      ...
      if(index > -1){
        _.remove(selectedUpload, {id: data.id});
      }else{
    ...
    const selected = _.findIndex(listUpload, {id: data.id});
    this.state.listUpload[selected].selectedData = checked;  
  ...
  closeModal = () => {
    const { challenge, content } = this.props;  
    this.setState({ openModalPreview: false, listUpload: this.state.listAll}, () => {  this.state.inputSearch ? this.handleSearch(): '';});
  }

  closeModalMultiReward = () => {
    const { challenge, content } = this.props;   
    {this.state.listUpload.map((data, i) => {
      this.state.listUpload[i].selectedData = false;
    }
    )}
    this.setState(({
      selectedUpload: [],
      openModalReward: false,
      listUpload: this.state.listAll,
      inputSearch: null
    }));
  ...
  handleSearch = (e) => {    
    ...
    !value ? value = '': '';
    let tempName = [], tempFullName = [], tempEmail = [], tempHashtags = [], tempGoogleTags = [];
    ...
    tempHashtags = listAll.filter((data) => data.hashtags ? data.hashtags.toLowerCase().includes(value.toLowerCase()): false);
    tempGoogleTags = listAll.filter((data) => data.googleHashtags ? data.googleHashtags.toLowerCase().includes(value.toLowerCase()): false);
    let result = _.union(tempName, tempFullName, tempEmail, tempHashtags, tempGoogleTags);
  ...
  sourceChanged = (value) =>{
    ...
    if (value.length == 0) {
      ...
      value.map((data) => {
        if(data == "INSTAGRAM"){
          temp = list.filter((obj) => obj.type.includes('instagram'));
          result = _.union(result, temp);
        } else {
          temp = list.filter((obj) => !obj.type.includes('instagram'));
          result = _.union(result, temp);
    ...
    this.setState({ listSource: result, sourceValue: value},this.getListFilter);   
  }
  ...
  confirmMultiple = type => {
    ...
    if(type){
      ...
      this.setState({ selectedUpload: [], loading: true})
    }else{
  ...
  getListFilter = () => {
    let result = [];
    result = _.intersection(this.state.listRating, this.state.listReward, this.state.listFlag, this.state.listSource);
    result = _.uniq(result);
    this.setState({listUpload: result});
  }
  ...
  handleLoading = (status) => {
    this.setState({loadingUpload: status});
  }
  handleLoading2 = (status) => {    
  ...
  callAPI = (pageIndex = 0) =>{
    const { token } = getCurrentUser();
    this.props.getListUploads(this.state.challengeId, token, this.state.activeType, pageIndex , this.state.maxRecords);
  }
  handleUpdate = (values) => { 
    ...
    return (
      <div id="container">
        <div id="wrapper">
          ...
          <MediaPopup handleLoading={this.handleLoading} handleLoading2={this.handleLoading2} listMedia={[]} handleSelect={this.handleSelect} currentChallenge={this.state.currentChallenge} isOpen={this.state.openMedia} closePopup={this.showModal} setSelected={this.setSelected}/>
          <NavHeaderClient
            title='Uploads'
          ...
          <div id="page-wrapper">
            <div className="uploads-dashboard">              
             ...
             <Scrollbars    ref="scrollbars"                                
                            onUpdate={this.handleUpdate} style={{ height: window.innerHeight - 108 - checkWidth}} renderThumbVertical={({ style, ...props }) =>
                        ...
                        <div className="content-left" style={{ minHeight: window.innerHeight - 108 + 'px' }}>                 
                          <div className="list-img">
                            {!this.state.loadingUpload && this.state.listUpload && this.state.listUpload.length > 0 ? 
                              ...
                              </div> : (!this.state.loadingUpload && !this.state.loading && this.state.listUpload && this.state.listUpload.length == 0? <span className="no-data" style={{margin: '10px'}}>No upload found</span> :'') }                            
                            </div>
                          </div>    
                        </SelectableGroup>               
                    </div>
                    <div className="col-lg-3 col-md-3 col-sm-12 col-xs-12 filter-list">
                      <FilterUpload showSource={true} listSource={this.state.listSource} sourceValue={this.state.sourceValue} sourceChanged={this.sourceChanged} ratingValue={this.state.ratingValue} flagValue={this.state.flagValue} rewardValue={this.state.rewardValue} sortAlpha={this.sortAlpha} sortDate={this.sortDate} activeType={this.state.activeType} ratingChanged={this.ratingChanged} flagChanged={this.flagChanged} rewardChanged={this.rewardChanged}/>
                    </div>
                  </div>
                </Scrollbars>               
              </div>
