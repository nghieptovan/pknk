...
export default class RewardsBody extends Component {
  constructor(props) {
    ...
    this.state={
      ...
      if(fileclients.importStatus !== nextfileclients.importStatus && nextfileclients.importStatus == 2){
        this.setState({ importSuccess: true, rewardList: nextfileclients.importResult, searchList: nextfileclients.importResult, listFlag: nextfileclients.importResult, listRating: nextfileclients.importResult, listReward: nextfileclients.importResult, originalList: nextfileclients.importResult});
        setTimeout(() => {
      ...
      client.getList !== nextClient.getList && nextClient.rewardList ? this.setState({ rewardList: nextClient.rewardList, searchList: nextClient.rewardList, listFlag: nextClient.rewardList, listRating: nextClient.rewardList, listReward: nextClient.rewardList, originalList: nextClient.rewardList, loadingUpload: false}) : '';
      if(!this.checkExpire()){
        client.loadingIcon !== nextClient.loadingIcon && nextClient.loadingIcon == 1 ? this.setState({loadingUpload: true}) :'';
        client.loadingIcon !== nextClient.loadingIcon && nextClient.loadingIcon != 1 && nextClient.rewardList ? this.setState({loadingUpload: false, rewardList: nextClient.rewardList, searchList: nextClient.rewardList, listFlag: nextClient.rewardList, listRating: nextClient.rewardList, listReward: nextClient.rewardList, originalList: nextClient.rewardList, loadingUpload: false}) :'';      
      } else {
  ...
  sourceChanged = (value) =>{
    ...
    let result = [];
    if (value.length == 0) {
      result = this.state.originalList;      
    } else {     
      let temp=[];     
      value.map((data) => {
        if(data == "INSTAGRAM"){
          temp = list.filter((obj) => obj.type ? obj.type.includes('instagram'): false);
    ...
    this.setState({ listSource: result, sourceValue: value},this.listHandler);   
  }
  ratingChanged = (value) =>{
    const list = this.state.originalList ;
    let result = [];
                  ...
                  <table className="table table-reward">
                      ...
                      <tbody style={{height: window.innerHeight - 90 + 'px'}}>     
                        {!this.state.loadingUpload &&  
                          (!searching && 
                          (this.state.rewardList && this.state.rewardList.length > 0) ?
                           this.state.rewardList.map(this.renderRewarded, this, '') :
                            <tr><td>No Upload Rewarded.</td></tr>)}
                      </tbody>
                  </table>
                  {this.state.loadingUpload && 
