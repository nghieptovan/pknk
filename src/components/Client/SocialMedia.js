...
export default class SocialMedia extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(nextProps.client.error) {
      ...
      error.includes('(code 190, subcode 463)') ? error = 'Current Facebook token has been expired, please logout and login again!': '';
  ...
  responseFacebook = response => {
    ...
    if(response.accessToken && !response.error){      
      ...
      this.setState({userData: response, loadingUpload: true });     
    }
  }
  closeModal = () => {
