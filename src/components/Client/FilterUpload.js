...
export default class FilterUpload extends Component {
    ...
    renderFilterStar = (data,index) => {       
      ...
      renderFilterSource = (data, index) => {       
        return(      
          <div className={`sources-item`} key={`sourceKey${index}`}>       
           <Checkbox value={data}/><span>{data}</span>
          </div>      
        )
      }      
    render() {
        ...
        const sources =['INSTAGRAM', 'DIRECT UPLOAD'];
        const rewards = this.state.rewards;  
        const {showPaid} = this.props;
        return (
            <div className="right-bar-list">
                <div className="sort">
                    <div className="title">
                        <span>Sort by: </span>
                    </div>
                ...
                {this.props.activeType !== "Pending" && <div>
                    ...
                    {this.props.showSource && <div className="sources">
                        <label className="sources-title">Upload source</label>
                        <div className="sources-list">
                        <CheckboxGroup
                            name="sources"
                            onChange={this.props.sourceChanged}
                            value={this.props.sourceValue}>
                            {sources.map(this.renderFilterSource, this, '')}
                        </CheckboxGroup>
                        </div>
                    </div>}                    
                </div>
