...
export default class DashboardBody extends Component {
  ...
  componentWillReceiveProps(nextProps){
    ...
    if(!this.checkExpire()){
      this.props.dashboard.loading !== nextProps.dashboard.loading && nextProps.dashboard.loading == 1? this.setState({loadingUpload: true})
      : '';
      this.props.dashboard.loading !== nextProps.dashboard.loading && nextProps.dashboard.loading != 1? this.setState({loadingUpload: false})
      : '';
    } else {
  ...
  changeUpload = (type) => {
    ...
    if(type === 1){
      ...
      this.setState({ currentUpload: clientInfo.dailyUploads, defaultUpload: type })
    }
  }
  ...
  handleNewUploads = () => {
    ...
    history.push(`/client/${this.state.shortName}/uploads`);
  }
  render() {
    if (this.state.clientInfo && !this.state.loadingUpload) { 
      ...
      let dataUpload = {};
      let dataKey ="";
      if(this.state.defaultUpload === 1){
        dataUpload= clientInfo.monthUploads;
        dataKey = "month";
      } else if(this.state.defaultUpload === 2){
        dataUpload= clientInfo.weekUploads;
        dataKey = "title";
      } else {
        dataUpload= clientInfo.dailyUploads;
        dataKey = "day";
      }     
          ...
          if (active) {
            ...
            if(label){
              return (
                ...
                <p className="label">{payload[0].payload.week ? payload[0].payload.week : (payload[0].payload.month ? payload[0].payload.month : payload[0].payload.day)}</p>
      ...
      const renderCustomizedLabel = (props) => {
        ...
        const positionY = height < 20 ? height + y : y + 15;
        let positionX;
        if(value < 10){
                      ...
                      <div className="overall">
                        ...
                        <div className="value-side">
                          <div className="view-row">
                            <span>{clientInfo.overalPerformance && clientInfo.overalPerformance.views || 0}</span>
                          </div>
                      ...
                      <div className="content-challenger">
                        <div className="row-fluid">
                          <div className="headerTable rowBody">
                            ...
                            <div className="col-lg-3 col-md-3 col-sm-3 col-xs-3 text-center">Pending Reviews</div>
                          </div>
                        </div>
