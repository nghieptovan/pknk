...
const renderField = ({ input, label, type, meta: { touched, error }, maxLength, autoFocus = false, className = '', readOnly = false}) => (
  <div>
    <input {...input} placeholder={label} type={type}  autoComplete="new-password" maxLength={maxLength ? maxLength :getMaxLengthField(input.name)} autoFocus = {autoFocus} className = {className} readOnly={readOnly}/>
