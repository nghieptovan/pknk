...
export default class RewardMultiUpload extends Component {
  ...
  renderMultiItem = (upload, data) => {
    ...
    const imageUri = upload.type && upload.type.includes('video') ? upload.thumbnail : upload.contentUri;
    const imagePreview = `${s3URL}${imageUri}` ;
    return (  
      <div className="item-slider-multi" key={`multiitem${data}`}> 
        <div className="img-item">   
            {upload.type && upload.type.includes('video') && <div className="video-icon">
  ...
  renderFirstItem = () => {
    ...
    if(data){
      ...
      const imageUri = item.type && item.type.includes('video') ? item.thumbnail : item.contentUri;
      const imagePreview = `${s3URL}${imageUri}`;
      return(
          <LazyLoad className="cover-img" key={'itemuploadFirst'} style={{ height: '100%' }}>
          <div> 
          {item.type && item.type.includes('video') && <div className="video-icon">
              <i className="icon-play-icon"></i>
          </div>}
  ...
  closeModal = () => {   
    this.setState({rewardAllNow: false, loading: false, selectRewardType: this.props.activeChallenge.rewardType ? this.props.activeChallenge.rewardType : 'Cash', selectRewardLevel : null }); 
    this.closeRewardAllNow();
    this.props.closeModalMultiReward();
    this.props.rewardDone();      
  };
          ...
          {!this.state.rewarded &&
            ...
            <form>
              ...
              <div>
                ...
                <div className="row endline" style={{ marginTop: '20px', marginBottom: '5px', height: '31px' }}>
