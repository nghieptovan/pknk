...
export default class CreateChallenge extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    this.props.form.challengeCreate && this.props.form.challengeCreate.syncErrors && this.props.form.challengeCreate.syncErrors.socialPost ? this.setState({requiredSocial: ''}) : '';
  }
  closeModal = () => {
    this.props.reset();
    ...
    if (this.state.currentStep === 3) {
      history.push(`/client/${this.props.shortName}/challenges`);
    }
  };
  ...
  onDrop = (files, type) => {
    ...
    if(fileType === 'image'){
      this.setState({ fileTypeError: '', defaultImage:'' });
      let reader = new FileReader(); 
      const EXIF = require('exif-js');        
      let css = {width: '100%', height: '100%', objectFit: 'contain'};   
      EXIF.getData(files[0],  () => { 
        let transform = exif2css(EXIF.getAllTags(files[0]).Orientation).transform;
        let transformOrigin = exif2css(EXIF.getAllTags(files[0]).Orientation)["transform-origin"];    
        transform ?  css.transform = transform : '';
        transformOrigin ?css.transformOrigin =  transformOrigin : '';   
        reader.readAsDataURL(files[0]); 
        reader.onload = () => {       
            this.setState({ imageContent: reader.result.split(',')[1], imageFileName: files[0].name, defaultImage: `${files[0].preview}`, css })              
          }; 
      })            
    }else{
    ...
    if (this.props.valid) {
      ...
      if (isOk) {
        const dataInput = {
          ...
          // entribeUri: "/"+this.props.shortName.replace(/\s/g, '') + '/'+encodeURIComponent(this.props.fields.challengeName.trim().replace(/\s/g, '%20').replace(/\./g, 'dotC'))+'/detail',
          status: type,
          hashtags: tags
        }
  ...
  onChangeInput = tag => {
    ...
    if (/\s/.test(tag) && tag.trim().length > 0) {
  ...
  handleChange = (data) => {   
    ...
    if(index >= 0){ 
      ...
      this.setState({tags: data, errorTags: ''});
    }
  }  
  ...
  render() {         
   const { facebookToken } = getCurrentUser();
    ...
    // this.props.fields.startDate ?console.log((new Date(Date.parse(this.props.fields.startDate.replace(/-/gm, '/'))))) : ''  
    let currentDate = new Date().getTime();
    let startDate = this.props.fields.startDate ? Date.parse(this.props.fields.startDate.replace(/-/gm, '/')) : 0;    
    ...
    let embeddedCode = this.props.challenge.createdChallenge ? `<iframe src='${domain}${"/widget/"+this.props.challenge.createdChallenge.id}'></iframe>`:'';
    // let shortLink = this.props.challenge.createdChallenge && this.props.challenge.createdChallenge.shortenedLink ? `${domain}${'/url/'+this.props.challenge.createdChallenge.shortenedLink}` : '';
    let currentUser = getCurrentUser();
        let shortName2 = currentUser.brandName.replace(/[^a-zA-Z0-9\ ]/g, '').toLowerCase() || '';  
    let shortLink = this.props.challenge.createdChallenge && this.props.challenge.createdChallenge.name ? `${domain}${"/"+shortName2.replace(/\s/g, '%20') + '/'+encodeURIComponent(this.props.challenge.createdChallenge.name.trim().replace(/\s/g, '%20').replace(/\./g, 'dotC'))+'/upload'}` : '';
    return (
      <div>
        <Modal
        ...
        >
          <div className="text-center" style={{lineHeight: '50px', marginBottom: '5px'}}>
            ...
            <form>
              <div className={this.state.currentStep === 1 ? 'showStep' : 'hiddenStep'}>
                ...
                <div className="row" style={{ height: '130px' }}>
                  ...
                  <div className="inputrow col-md-6 col-lg-6 col-sm-6 col-xs-6">
                    ...
                    <div className="row">
                      {this.state.defaultImage && <div className='image-holder'><img style={this.state.css} src={this.state.defaultImage} /></div>}
                    </div>
                  </div>
                </div>
              ...
              <div className={this.state.currentStep === 2 ? 'showStep' : 'hiddenStep'}>
                ...
                <div className="row" style={{ height: 'auto' }}>
                  ...
                  <div className="inputrow col-md-6 col-lg-6 col-sm-12 col-xs-12" style={{ height: 'auto', marginTop: '5px' }}>
                ...
                <div className="row" style={{ height: 'auto' }}>
                  <div className="inputrow col-md-12 col-lg-12 col-sm-12 col-xs-12">
                    <div className="hashtag">
                      <TagsInput value={this.state.tags} inputValue={this.state.tag} onChangeInput={this.onChangeInput} onChange={this.handleChange} inputProps={{placeholder: "Add your own hashtags here"}} />                      
                    </div>
              ...
              <div className={this.state.currentStep === 3 ? 'showStep' : 'hiddenStep'}>
                <div className="row social-share-row">
                  <div className="inputrow col-md-6 col-lg-6 col-sm-12 col-xs-12 nopadding50 leftEmbedded" style={{ height: '135px', top: '30px', marginTop: '40px' }}>
                    ...
                    />
                    <p className="text-danger">{this.props.form.challengeCreate && this.props.form.challengeCreate.syncErrors && this.props.form.challengeCreate.syncErrors.socialPost ? '' : this.state.requiredSocial}</p>
                  </div>
                ...
                <div className="row social-share-row">                  
                  ...
                  <div className="col-md-6 col-lg-6 col-sm-12 col-xs-12 left-social-row">
                    <label className="entribe-uri">{shortLink}</label>   
                  </div>
