...
export const getListUploads  = (challengeId, token, status, page, maxRecords) => (
  {
      types: [types.CREATOR_GET_LIST_UPLOAD, types.CREATOR_GET_LIST_UPLOAD_SUCCESS, types.CREATOR_GET_LIST_UPLOAD_FAIL],
      promise: (client) => client.get(`/api/content/get/${challengeId}/${status}?access_token=${token}&page=${page}&maxRecords=${maxRecords}`)
  }
)
