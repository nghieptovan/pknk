...
export const getChallengeForReward  = (token, id) => (
    {
        types: [types.GET_CHALLENGE_FOR_UPLOAD, types.GET_CHALLENGE_FOR_UPLOAD_SUCCESS, types.GET_CHALLENGE_FOR_UPLOAD_FAIL],
        promise: (client) => client.get(`/api/challenge/get/${id}?access_token=${token}`)
    }
)
...
export const getChallengeForWidgetByBrandNameAndChallengeName  = (brandName, name) => (
    {
        types: [types.GET_CHALLENGE_BY_ID, types.GET_CHALLENGE_BY_ID_SUCCESS, types.GET_CHALLENGE_BY_ID_FAIL],
        promise: (client) => client.get(`/api/challenge/get/widget/0?brandName=${brandName}&challengeName=${name}`)
    }
)
