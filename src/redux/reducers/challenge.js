...
export default function challenge(state = initState, action) {
  ...
  case types.GET_LIST_CHALLENGES_FOR_UPLOAD_FAIL:
    ...
    case types.GET_CHALLENGE_FOR_UPLOAD:
    return {
      ...state,
      getChallenge: true,
      challengeInfo: null,
      error: '',         
      loadingChallengeInfo: 1        
    }
  case types.GET_CHALLENGE_FOR_UPLOAD_SUCCESS:      
    return {
      ...state,
      getChallenge: false,
      challengeInfo: action.result,
      error: '',        
      loadingChallengeInfo: 2
    }
  case types.GET_CHALLENGE_FOR_UPLOAD_FAIL:
    return {
      ...state,
      getChallenge: false,
      challengeInfo: null,
      error: action.error,       
      loadingChallengeInfo: 3
  }       
...
let editData = [];
let count = 1;    
if(data.weeklyUploads){       
  _.forEach(data.weeklyUploads, (value) => {
    if(count >= 0){
